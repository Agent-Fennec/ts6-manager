...
function sortGroups(groups: any[]): any[] {
  return [...groups].sort((a, b) => {
    const sortDiff = (parseInt(a.sortid) || 0) - (parseInt(b.sortid) || 0);
    return sortDiff !== 0 ? sortDiff : String(a.name).localeCompare(String(b.name));
  });
}
...
export function useServerGroups() {
  ...
  return useQuery({
    ...
    enabled: !!c && !!s,
    select: (data: any) => (Array.isArray(data) ? sortGroups(data) : data),
  });
}
