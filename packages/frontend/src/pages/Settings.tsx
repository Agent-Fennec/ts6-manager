...
import { Settings as SettingsIcon, Users, Server, Plus, Trash2, Pencil, TestTube, Check, X, Lock, KeyRound, PlayCircle, Upload, FileText } from 'lucide-react';
    ...
    <div className="space-y-5">
      ...
      <Tabs defaultValue="account">
        <TabsList>
          ...
          {isAdmin && <TabsTrigger value="youtube"><PlayCircle className="h-3.5 w-3.5 mr-1" /> YouTube</TabsTrigger>}
        </TabsList>
...
function ConnectionsTab() {
  ...
  const [form, setForm] = useState({ name: '', host: '', webqueryPort: '10080', apiKey: '', useHttps: false, sshPort: '10022', sshUsername: '', sshPassword: '', queryBotNickname: '', sshBotNickname: '', queryBotChannel: '' });

  const serverList = useMemo(() => (Array.isArray(servers) ? servers : []), [servers]);

  if (isLoading) return <PageLoader />;

  const resetForm = () => setForm({ name: '', host: '', webqueryPort: '10080', apiKey: '', useHttps: false, sshPort: '10022', sshUsername: '', sshPassword: '', queryBotNickname: '', sshBotNickname: '', queryBotChannel: '' });
  ...
  const openEdit = (server: any) => {
    setForm({
      ...
      queryBotNickname: server.queryBotNickname || '',
      sshBotNickname: server.sshBotNickname || '',
      queryBotChannel: server.queryBotChannel || '',
    });
    setEditId(server.id);
    setShowAdd(true);
  };
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div><Label className="text-xs">SSH Password</Label><Input type="password" value={form.sshPassword} onChange={(e) => setForm({ ...form, sshPassword: e.target.value })} placeholder={editId ? '(unchanged)' : ''} /></div>
            </div>
            <div>
              <Label className="text-xs">Bot Nickname</Label>
              <Input
            ...
            <div>
              <Label className="text-xs">SSH Bot Nickname</Label>
              <Input
            ...
            <div>
              <Label className="text-xs">Query Bot Channel</Label>
              {editId ? (
                <ChannelSelect
                  configId={editId}
                  sid={1}
                  placeholder="752"
                  value={form.queryBotChannel}
                  onChange={(v) => setForm({ ...form, queryBotChannel: v })}
                />
              ) : (
                <Input
                  value={form.queryBotChannel}
                  onChange={(e) => setForm({ ...form, queryBotChannel: e.target.value })}
                  placeholder="752"
                />
              )}
              <p className="text-[10px] text-muted-foreground mt-1">Channel the SSH query bot joins on connect</p>
            </div>
          </div>
          <DialogFooter>
