...
import {
  ...
  Moon, Timer, Megaphone, Award, Shield, Database,
  Music, Volume2, LogIn, LogOut, Pause, SkipForward, Navigation, Mic, Sparkles, Key,
} from 'lucide-react';
...
const SMART_ACTION_NODES: NodeTypeDef[] = [
  ...
  { type: 'action_valkeyGet', label: 'Valkey Get', icon: Database, color: 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30', handles: { inputs: ['in'], outputs: ['out'] } },
  { type: 'action_valkeySet', label: 'Valkey Set', icon: Database, color: 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30', handles: { inputs: ['in'], outputs: ['out'] } },
  { type: 'action_valkeyDelete', label: 'Valkey Delete', icon: Database, color: 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30', handles: { inputs: ['in'], outputs: ['out'] } },
  { type: 'action_groupRemoveAll', label: 'Remove All Groups', icon: Users, color: 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30', handles: { inputs: ['in'], outputs: ['out'] } },
  { type: 'action_groupRestoreList', label: 'Restore Groups', icon: Users, color: 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30', handles: { inputs: ['in'], outputs: ['out'] } },
];
...
const LOGIC_NODES: NodeTypeDef[] = [
  ...
  { type: 'action_generateToken', label: 'Generate Token', icon: Key, color: 'bg-teal-500/20 text-teal-400 border-teal-500/30', handles: { inputs: ['in'], outputs: ['out'] } },
  { type: 'action_setClientChannelGroup', label: 'Set Channel Group', icon: Users, color: 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30', handles: { inputs: ['in'], outputs: ['out'] } },
];
...
export default function BotEditor() {
  ...
  const configId = bot?.serverConfigId ?? null;
  const sid = bot?.virtualServerId ?? null;
                ...
                <div>
                    ...
                    <div>
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="notifyclientupdated">Client Status Updated (any)</SelectItem>
                          <SelectItem value="client_went_away">Client Went Away</SelectItem>
                          <SelectItem value="client_came_back">Client Came Back</SelectItem>
                          <SelectItem value="client_mic_muted">Client Mic Muted</SelectItem>
                          <SelectItem value="client_mic_unmuted">Client Mic Unmuted</SelectItem>
                          <SelectItem value="client_sound_muted">Client Sound Muted</SelectItem>
                          <SelectItem value="client_sound_unmuted">Client Sound Unmuted</SelectItem>
                          <SelectItem value="client_mic_disabled">Client Mic HW Disabled</SelectItem>
                          <SelectItem value="client_mic_enabled">Client Mic HW Enabled</SelectItem>
                          <SelectItem value="client_sound_disabled">Client Sound HW Disabled</SelectItem>
                          <SelectItem value="client_sound_enabled">Client Sound HW Enabled</SelectItem>
                          <SelectItem value="client_recording_started">Client Recording Started</SelectItem>
                          <SelectItem value="client_recording_stopped">Client Recording Stopped</SelectItem>
                          <SelectItem value="client_nickname_changed">Client Nickname Changed</SelectItem>
                          <SelectItem value="notifytextmessage">Text Message</SelectItem>
                          <SelectItem value="notifychannelcreated">Channel Created</SelectItem>
                          <SelectItem value="notifychanneledited">Channel Edited</SelectItem>
                          <SelectItem value="notifychanneldeleted">Channel Deleted</SelectItem>
                          <SelectItem value="notifychannelmoved">Channel Moved</SelectItem>
                          <SelectItem value="notifychanneldescriptionchanged">Channel Desc Changed</SelectItem>
                          <SelectItem value="notifychannelpasswordchanged">Channel Password Changed</SelectItem>
                          <SelectItem value="notifyserveredited">Server Edited</SelectItem>
                          <SelectItem value="notifytokenused">Privilege Key Used</SelectItem>
                        </SelectContent>
                      </Select>
                      <Separator className="my-3" />
                      <div>
                        <p className="text-[10px] font-semibold text-muted-foreground mb-2">FILTERS</p>
                        {((selectedNodeData.config.filters as { field: string; value: string }[] | undefined) ?? []).map((f, i) => (
                          <div key={i} className="flex gap-2 items-center mt-2">
                            <Input
                              className="h-7 text-xs font-mono-data flex-1"
                              placeholder="field"
                              value={f.field}
                              onChange={(e) => {
                                const updated = (selectedNodeData.config.filters as { field: string; value: string }[]).map((r, ri) =>
                                  ri === i ? { ...r, field: e.target.value } : r
                                );
                                setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, filters: updated } } : n));
                              }}
                            />
                            <Input
                              className="h-7 text-xs font-mono-data flex-1"
                            ...
                            <Button
                              ...
                              onClick={() => {
                                const updated = (selectedNodeData.config.filters as { field: string; value: string }[]).filter((_, ri) => ri !== i);
                                setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, filters: updated } } : n));
                              }}
                            >
                              ×
                        ...
                        <Button
                          ...
                          onClick={() => {
                            const current = (selectedNodeData.config.filters as { field: string; value: string }[] | undefined) ?? [];
                            setNodes((prev) => prev.map((n) => n.id === selectedNode
                              ? { ...n, config: { ...n.config, filters: [...current, { field: '', value: '' }] } }
                              : n
                            ));
                          }}
                        >
                        ...
                        {((selectedNodeData.config.filters as { field: string; value: string }[] | undefined) ?? []).length === 0 ? (
                          <p className="text-[10px] text-muted-foreground mt-1">No filters — flow fires on every matching event.</p>
                        ) : (
                          <p className="text-[10px] text-muted-foreground mt-1">All filters must match for the flow to fire.</p>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                      ...
                      <div>
                        ...
                        <ChannelSelect
                          configId={configId} sid={sid}
                          placeholder="42"
                          value={selectedNodeData.config.channelId || ''}
                          onChange={(v) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, channelId: v } } : n))}
                        />
                    ...
                    <div className="space-y-2">
                      ...
                      {selectedNodeData.config.targetMode !== 'server' && (
                        <div>
                          ...
                          {selectedNodeData.config.targetMode === 'channel' ? (
                            <ChannelSelect
                              configId={configId} sid={sid}
                              placeholder="Channel ID or {{event.ctid}}"
                              value={selectedNodeData.config.target || ''}
                              onChange={(v) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, target: v } } : n))}
                            />
                          ) : (
                            <Input
                              className="h-7 text-xs mt-1 font-mono-data"
                              placeholder="Client ID or {{event.clid}}"
                              value={selectedNodeData.config.target || ''}
                  ...
                  {selectedNodeData.type === 'action_move' && (
                    <div>
                      ...
                      <ChannelSelect
                        ...
                        onChange={(v) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, cid: v } } : n))}
                      />
                      <p className="text-[9px] text-muted-foreground/60 mt-0.5">
                    ...
                    <div className="space-y-2">
                      ...
                      <div>
                        ...
                        <ChannelSelect
                          configId={configId} sid={sid}
                          placeholder="0"
                          value={selectedNodeData.config.cpid || ''}
                          onChange={(v) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, cpid: v } } : n))}
                        />
                      </div>
                      <div>
                    ...
                    <div className="space-y-2">
                      <div>
                        ...
                        <ChannelSelect
                          configId={configId} sid={sid}
                          placeholder="10"
                          value={selectedNodeData.config.afkChannelId || ''}
                          onChange={(v) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, afkChannelId: v } } : n))}
                        />
                      </div>
                      <div>
                      ...
                      <div className="flex items-center gap-2 pt-1">
                        <input type="checkbox" id="checkMuteState" className="h-3.5 w-3.5 accent-primary" checked={!!selectedNodeData.config.checkMuteState} onChange={(e) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, checkMuteState: e.target.checked } } : n))} />
                        <Label htmlFor="checkMuteState" className="text-[10px] text-muted-foreground cursor-pointer">Move clients with both mic &amp; speakers muted (regardless of idle time)</Label>
                      </div>
                    </div>
                  )}
                    ...
                    <>
                      <div>
                        ...
                        <ChannelSelect
                          configId={configId} sid={sid}
                          placeholder="19"
                          value={selectedNodeData.config.parentChannelId || ''}
                          onChange={(v) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, parentChannelId: v } } : n))}
                        />
                      </div>
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Protected Channel IDs</Label>
                        <ChannelMultiSelect
                          configId={configId} sid={sid}
                          placeholder="Select channels to protect…"
                          value={selectedNodeData.config.protectedChannelIds || ''}
                          onChange={(v) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, protectedChannelIds: v } } : n))}
                        />
                  ...
                  {selectedNodeData.type === 'action_generateToken' && (
                    <div className="space-y-2">
                      <p className="text-[10px] font-semibold text-muted-foreground mb-2">CONFIGURATION</p>
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Token Type</Label>
                        <Select
                          value={selectedNodeData.config.tokenType || '0'}
                          onValueChange={(v) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, tokenType: v } } : n))}
                        >
                          <SelectTrigger className="h-7 text-xs mt-1"><SelectValue placeholder="Select type..." /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="0">Server Group Token</SelectItem>
                            <SelectItem value="1">Channel Group Token</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Group ID</Label>
                        <Input className="h-7 text-xs mt-1 font-mono-data" placeholder="sgid or cgid" value={selectedNodeData.config.groupId || ''} onChange={(e) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, groupId: e.target.value } } : n))} />
                      </div>
                      {(selectedNodeData.config.tokenType === '1') && (
                        <div>
                          <Label className="text-[10px] text-muted-foreground">Channel ID</Label>
                          <ChannelSelect
                            configId={configId} sid={sid}
                            ...
                            onChange={(v) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, channelId: v } } : n))}
                          />
                          <p className="text-[10px] text-muted-foreground/60 mt-1">Required for channel group tokens</p>
                        </div>
                      )}
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Store result as</Label>
                        <Input className="h-7 text-xs mt-1 font-mono-data" placeholder="result" value={selectedNodeData.config.storeAs || ''} onChange={(e) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, storeAs: e.target.value } } : n))} />
                        <p className="text-[10px] text-muted-foreground mt-1">Use as: {'{{temp.' + (selectedNodeData.config.storeAs || 'result') + '}}' }</p>
                      </div>
                    </div>
                  )}

                  {selectedNodeData.type === 'action_setClientChannelGroup' && (
                    <div className="space-y-2">
                      <p className="text-[10px] font-semibold text-muted-foreground mb-2">CONFIGURATION</p>
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Channel Group ID</Label>
                        <Input className="h-7 text-xs mt-1 font-mono-data" placeholder="cgid" value={selectedNodeData.config.channelGroupId || ''} onChange={(e) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, channelGroupId: e.target.value } } : n))} />
                      </div>
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Channel ID</Label>
                        <ChannelSelect
                      ...
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Store result as (optional)</Label>
                        <Input className="h-7 text-xs mt-1 font-mono-data" placeholder="result" value={selectedNodeData.config.storeAs || ''} onChange={(e) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, storeAs: e.target.value } } : n))} />
                        <p className="text-[10px] text-muted-foreground mt-1">Use as: {'{{temp.' + (selectedNodeData.config.storeAs || 'result') + '}}' }</p>
                      </div>
                    </div>
                  ...
                  {selectedNodeData.type === 'action_valkeyGet' && (
                    <div className="space-y-2">
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Key</Label>
                        <Input
                          className="h-7 text-xs mt-1 font-mono-data"
                          placeholder="vpn:{{event.client_ip}}"
                        ...
                        />
                        <p className="text-[9px] text-muted-foreground/60 mt-0.5">Supports template variables</p>
                      </div>
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Store As</Label>
                        <Input
                          className="h-7 text-xs mt-1 font-mono-data"
                          placeholder="cachedResult"
                          value={selectedNodeData.config.storeAs ?? ''}
                          onChange={(e) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, storeAs: e.target.value } } : n))}
                        />
                        <p className="text-[9px] text-muted-foreground/60 mt-0.5">
                          {'Result in {{temp.'}{selectedNodeData.config.storeAs || 'cachedResult'}{'}} — null if key missing'}
                        </p>
                      </div>
                    </div>
                  )}

                  {selectedNodeData.type === 'action_valkeySet' && (
                    <div className="space-y-2">
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Key</Label>
                        <Input
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, value: e.target.value } } : n))}
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          className="h-7 text-xs mt-1 font-mono-data"
                          placeholder="86400"
                          value={selectedNodeData.config.ttlSeconds ?? ''}
                          onChange={(e) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, ttlSeconds: e.target.value } } : n))}
                        />
                        <p className="text-[9px] text-muted-foreground/60 mt-0.5">Leave empty for no expiry</p>
                      </div>
                    </div>
                  )}

                  {selectedNodeData.type === 'action_valkeyDelete' && (
                    <div className="space-y-2">
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Key</Label>
                        <Input
                  ...
                  {selectedNodeData.type === 'action_groupRemoveAll' && (
                    <div>
                      <Label className="text-[10px] text-muted-foreground">Keep Group IDs (comma-separated)</Label>
                      <Input
                        className="h-7 text-xs mt-1 font-mono-data"
                        placeholder="143"
                        value={selectedNodeData.config.keepGroupIds ?? ''}
                        onChange={(e) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, keepGroupIds: e.target.value } } : n))}
                      />
                      <p className="text-[9px] text-muted-foreground/60 mt-0.5">Groups to retain — all others are removed</p>
                    </div>
                  )}

                  {selectedNodeData.type === 'action_groupRestoreList' && (
                    <div className="space-y-2">
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Group IDs (template)</Label>
                        <Input
                          className="h-7 text-xs mt-1 font-mono-data"
                          placeholder="{{temp.savedRoles}}"
                          value={selectedNodeData.config.groupIds ?? ''}
                          onChange={(e) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, groupIds: e.target.value } } : n))}
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          className="h-7 text-xs mt-1 font-mono-data"
                          placeholder="143"
                          value={selectedNodeData.config.excludeGroupIds ?? ''}
                          onChange={(e) => setNodes((prev) => prev.map((n) => n.id === selectedNode ? { ...n, config: { ...n.config, excludeGroupIds: e.target.value } } : n))}
                        />
                        <p className="text-[9px] text-muted-foreground/60 mt-0.5">Groups to skip when restoring (e.g. jail group)</p>
                      </div>
                    </div>
                  )}
