...
export const BOT_TEMPLATES: BotTemplate[] = [
  ...
  },
  {
    id: 'anti-vpn',
    name: 'Anti-VPN',
    description: 'Checks connecting clients against iphub.info and kicks/bans VPN users.',
    category: 'integration',
    icon: Globe,
    configFields: [
      { key: 'apiKey', label: 'iphub.info API Key', type: 'text', placeholder: 'Your iphub.info API key', required: true },
      { key: 'action', label: 'Action for VPN', type: 'select', defaultValue: 'kick', options: [{ label: 'Kick', value: 'kick' }, { label: 'Ban (1h)', value: 'ban' }] },
    ],
    flowDataFactory: (cfg) => {
      resetIds();
      const n1 = nid(), n2 = nid(), n3 = nid(), n4 = nid(), n5 = nid(), n6 = nid();
      const vpnUrl = 'https://v2.api.iphub.info/ip/{{temp.clientInfo.connection_client_ip}}';
      const headers = { 'X-Key': cfg.apiKey || '' };
      const actionNode = cfg.action === 'ban'
        ? makeNode(n6, 'action_ban', 'Ban VPN', { time: 3600, reason: 'VPN detected' }, 1020, 0)
        : makeNode(n6, 'action_kick', 'Kick VPN', { reasonid: '5', reason: 'VPN detected' }, 1020, 0);

      return {
        nodes: [
          makeNode(n1, 'trigger_event', 'Client Enter', { eventName: 'notifycliententerview' }, 60, 80),
          makeNode(n2, 'condition', 'Is Human?', { expression: 'event.client_type == 0' }, 300, 80),
          makeNode(n3, 'action_webquery', 'Get Client IP', { command: 'clientinfo', params: { clid: '{{event.clid}}' }, storeAs: 'clientInfo' }, 540, 80),
          makeNode(n4, 'action_httpRequest', 'Check VPN API', { url: vpnUrl, method: 'GET', headers, storeAs: 'vpn', cacheKey: '{{temp.clientInfo.connection_client_ip}}', cacheTtlSeconds: 86400 }, 780, 40),
          makeNode(n5, 'condition', 'Is VPN?', { expression: 'temp.vpn.block == 1' }, 780, 120),
          actionNode,
        ],
        edges: [
          makeEdge(eid(), n1, n2),
