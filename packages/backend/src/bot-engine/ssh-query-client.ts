...
export class SshQueryClient extends EventEmitter {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const connectTimeout = setTimeout(() => {
        if (!settled) {
          ...
          logger.error(`[SshQueryClient] ${err.message}`);
          this.emit('error', err);
          reject(err);
      ...
      this.ssh.on('ready', () => {
        this.ssh!.shell(false, (err, channel) => {
          ...
          channel.stderr.on('data', (data: Buffer) => {
            logger.error(`[SshQueryClient] stderr: ${data.toString('utf-8')}`);
          });
        });
      });
      ...
      this.ssh.on('error', (err: Error) => {
        ...
        if (isAuthError) {
          ...
          logger.error(`[SshQueryClient] Fatal auth error for ${this.options.host}:${this.options.port}: ${err.message}`);
        }
        this.emit('error', err);
        if (!settled) {
          settled = true;
  ...
  async registerEvents(sid: number, channelId?: string, nickname?: string): Promise<void> {
    logger.info(`[SshQueryClient] Registering events for sid=${sid} on ${this.options.host}`);
    await this.executeCommand(`use sid=${sid}`);

    // Set nickname so the bot is identifiable, and mark as query client type
    try {
      const escapedNickname = (nickname ?? 'TS6 Query').replace(/ /g, '\\s');
      await this.executeCommand(`clientupdate client_nickname=${escapedNickname}`);
    } catch { }
    ...
    for (const eventType of TS_EVENT_TYPES) {
      ...
      try {
        ...
        if (!err.message?.includes('516')) {
          logger.warn(`[SshQueryClient] Failed to register event ${eventType}: ${err.message}`);
        }
      }
    }

    logger.info(`[SshQueryClient] Events registered for sid=${sid}`);

    // Optionally move the query bot into a designated channel
    if (channelId) {
      try {
        ...
        if (clid) {
          ...
          logger.info(`[SshQueryClient] Query bot moved to channel ${channelId}`);
        } else {
          logger.warn('[SshQueryClient] whoami did not return clid; cannot move query bot to channel');
        }
      } catch (err: any) {
        logger.warn(`[SshQueryClient] Failed to move query bot to channel ${channelId}: ${err.message}`);
      }
    }
  }

  async registerCommandListener(sid: number, channelId: number): Promise<void> {
    logger.info(`[SshQueryClient] Registering command listener for sid=${sid}, channelId=${channelId} on ${this.options.host}`);
    ...
    try {
      ...
      if (clid) {
        ...
        logger.warn('[SshQueryClient] whoami did not return clid; cannot clientmove');
      }
    } catch (err: any) {
      logger.warn(`[SshQueryClient] Failed to move query client to channel ${channelId}: ${err.message}`);
    }
    ...
    try {
      ...
      if (!err.message?.includes('516')) {
        logger.warn(`[SshQueryClient] Failed to register textchannel for channel ${channelId}: ${err.message}`);
      }
    }

    logger.info(`[SshQueryClient] Command listener ready for sid=${sid}, channelId=${channelId}`);
  }
    ...
    this.keepaliveTimer = setInterval(() => {
      if (this.connected) {
        this.executeCommand('whoami', 5000)
          ...
          .catch((err) => {
            consecutiveFailures++;
            logger.warn(`[SshQueryClient] Keepalive failed for ${this.options.host}:${this.options.port} (${consecutiveFailures}/3): ${err.message}`);
            if (consecutiveFailures >= 3) {
              logger.error(`[SshQueryClient] Keepalive failed 3 times, forcing disconnect for ${this.options.host}:${this.options.port}`);
  ...
  private scheduleReconnect(): void {
    ...
    logger.info(`[SshQueryClient] Reconnecting to ${this.options.host}:${this.options.port} in ${delay}ms (attempt ${this.reconnectAttempt + 1})`);
    ...
    this.reconnectTimer = setTimeout(async () => {
      ...
      try {
        ...
        logger.error(`[SshQueryClient] Reconnect failed: ${err.message}`);
