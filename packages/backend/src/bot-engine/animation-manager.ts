...
export class AnimationManager {
  ...
  startAnimation(
    ...
    const tick = async () => {
      try {
        ...
        if (!err.message?.includes('304')) { // 304 = name unchanged, ignore
          logger.error(`[AnimationManager] Flow ${flowId} channel update error: ${err.message}`);
        }
      }
    };
    ...
    logger.info(`[AnimationManager] Started animation for flow ${flowId}: style=${config.style}, interval=${config.intervalSeconds}s, channel=${config.channelId}`);
  }
  ...
  stopAnimation(flowId: number): void {
    ...
    if (anim) {
      ...
      logger.info(`[AnimationManager] Stopped animation for flow ${flowId}`);
    }
  }
