...
function resolveVideoUrl(url: string, maxHeight: number = 720): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      ...
      logger.info(`[VideoResolve] Resolved: ${url.substring(0, 60)}... → direct URL`);
      resolve(directUrl);
    });
...
export class VoiceBot extends EventEmitter {
  ...
  async startVideoStream(source: string, preset?: string, framerate?: number, bitrate?: string): Promise<void> {
    ...
    if (sidecarUrl) {
      ...
      this.sidecarProc.on('exited', (code: number | null) => {
        logger.info(`[VoiceBot ${this.config.id}] Sidecar exited (code=${code})`);
        if (this._videoStreaming) {
    ...
    logger.info(`[VoiceBot ${this.config.id}] Sidecar ready`);
    ...
    logger.info(`[VoiceBot ${this.config.id}] Video stream started: ${stream.id}, source: ${source}`);
  ...
  async stopVideoStream(): Promise<void> {
    ...
    // Stop TS6 stream
    if (this.signaling && this._activeStreamId) {
      logger.info(`[VoiceBot ${this.config.id}] Sending stopstream: ${this._activeStreamId}`);
    ...
    logger.info(`[VoiceBot ${this.config.id}] Video stream stopped`);
    this.emit('videoStreamStopped');
    this.emit('statusChange', this._status);
  }
  ...
  async setVideoSource(source: string): Promise<void> {
    ...
    logger.info(`[VoiceBot ${this.config.id}] Video source changed: ${source}`);
  ...
  private setupSignalingListeners(): void {
    ...
    this.signaling.on('joinStreamRequest', (params: Record<string, string>) => {
      ...
      logger.info(`[VoiceBot ${this.config.id}] Viewer join request: clid=${viewerClid}`);
    ...
    this.signaling.on('streamClientLeft', (params: Record<string, string>) => {
      ...
      if (this._viewers.has(clid)) {
        logger.info(`[VoiceBot ${this.config.id}] Viewer left: clid=${clid}`);
      ...
      case 'answer':
        if (msg.sdp && msg.clid) {
          try {
            ...
            logger.error(`[VoiceBot ${this.config.id}] setAnswer error (clid=${msg.clid}): ${err.message}`);
          }
        }
        break;
      case 'ice_candidate':
        if (msg.candidate && msg.clid) {
          try {
            ...
            logger.error(`[VoiceBot ${this.config.id}] addIceCandidate error (clid=${msg.clid}): ${err.message}`);
          }
        }
        break;
      case 'reconnect':
        if (msg.clid && this._activeStreamId) {
          logger.info(`[VoiceBot ${this.config.id}] Reconnect from clid=${msg.clid}`);
  ...
  private async handleViewerJoin(viewerClid: number, streamId: string): Promise<void> {
    ...
    try {
      ...
      logger.info(`[VoiceBot ${this.config.id}] Viewer accepted: clid=${viewerClid} (${this._viewers.size} total)`);
      this.emit('videoViewerJoined', viewer);
    } catch (err: any) {
      logger.error(`[VoiceBot ${this.config.id}] handleViewerJoin error (clid=${viewerClid}): ${err.message}`);
