...
export class VoiceBotManager extends EventEmitter {
  ...
  private startErrors = new Map<number, string>();
  ...
  async start(): Promise<void> {
    ...
    logger.info(`[VoiceBotManager] Loading ${dbBots.length} music bot(s)...`);
    ...
    for (const dbBot of dbBots) {
      ...
      const config: VoiceBotConfig = {
        ...
        sidecarPort: (dbBot as Record<string, unknown>).sidecarPort as number ?? 9800,
        streamPreset: (dbBot as Record<string, unknown>).streamPreset as string ?? '720p',
      };
      ...
      if (dbBot.autoStart) {
        bot.start().catch((err: Error) => {
          logger.error(`[VoiceBotManager] Auto-start failed for bot ${dbBot.id}: ${err.message}`);
          this.startErrors.set(dbBot.id, err.message);
        });
      }
    }
  }
  ...
  private createBotInstance(config: VoiceBotConfig): VoiceBot {
    ...
    bot.on('error', (err: Error) => {
      logger.error(`[VoiceBotManager] Bot ${config.id} error: ${err.message}`);
    });
    ...
    bot.on('nowPlaying', (item: QueueItem) => {
      ...
      this.emit('nowPlaying', config.id, item);
    });
    ...
    bot.on('disconnected', () => {
      if (!bot.manuallyStopped) {
        logger.info(`[VoiceBotManager] Bot ${config.id}: unexpected disconnect, scheduling reconnect`);
    ...
    bot.on('fatalError', (msg: string) => {
      logger.error(`[VoiceBotManager] Bot ${config.id}: fatal error — ${msg}. No reconnect.`);
  ...
  async createBot(data: {
    ...
    const dbBot = await this.prisma.musicBot.create({
      data: {
        ...
        identityData,
        ...(data.nowPlayingChannelId !== undefined && { nowPlayingChannelId: data.nowPlayingChannelId }),
      },
    });
  ...
  getStartError(id: number): string | undefined {
    return this.startErrors.get(id);
  }
  ...
  private scheduleReconnect(botId: number): void {
    ...
    if (state.attempts >= MAX_RECONNECT_ATTEMPTS) {
      logger.error(`[VoiceBotManager] Bot ${botId}: max reconnect attempts (${MAX_RECONNECT_ATTEMPTS}) reached, giving up`);
    ...
    state.attempts++;
    logger.info(`[VoiceBotManager] Bot ${botId}: reconnect attempt ${state.attempts}/${MAX_RECONNECT_ATTEMPTS} in ${delay / 1000}s`);
  ...
  private async attemptReconnect(botId: number): Promise<void> {
    ...
    if (bot.status === 'error') {
      logger.info(`[VoiceBotManager] Bot ${botId}: in error state, aborting reconnect`);
      this.reconnectState.delete(botId);
      return;
    }
    ...
    try {
      ...
      await bot.start();
      logger.info(`[VoiceBotManager] Bot ${botId}: reconnected successfully after ${state.attempts} attempt(s)`);
      this.reconnectState.delete(botId);
    } catch (err: any) {
      logger.error(`[VoiceBotManager] Bot ${botId}: reconnect attempt ${state.attempts} failed: ${err.message}`);
