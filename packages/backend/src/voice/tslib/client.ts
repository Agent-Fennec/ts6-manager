...
export class Ts3Client extends EventEmitter {
  ...
  sendCommand(cmd: string): void {
    ...
    logger.info(`[TS3Client] Fragmenting ${cmdName}: ${data.length} bytes → ${chunks.length} fragments`);
  ...
  private handleChannelListFinished(): void {
    ...
    // Delay the move slightly — TS3 server sometimes hasn't fully seated the
    // new client by the time channellistfinished arrives, causing the clientmove
    // to fail silently. 1 second is enough in practice.
    const clientId = this.clientId;
    const resolvedCid = cid;
    setTimeout(() => {
      // Guard: connection may have dropped during the delay
      if (this.state !== "connected" || !this.clientId) return;
      const moveCmd = buildCommand("clientmove", {
        cid: resolvedCid,
        clid: clientId,
        cpw: this.opts.channelPassword ?? "",
      });
      this.emit("debug", `Moving to channel "${target}" (cid=${resolvedCid})`);
