  ...
  try {
    ...
    const dbBots = await prisma.musicBot.findMany({
      select: {
        ...
        name: true,
        serverConfigId: true,
        serverConfig: { select: { id: true, name: true, host: true } },
        nickname: true,
        defaultChannel: true,
        voicePort: true,
        volume: true,
        autoStart: true,
        nowPlayingChannelId: true,
        createdAt: true,
      },
      orderBy: { id: 'asc' },
    });
    ...
    const runtimeMap = new Map(runtimeInfo.map((b) => [b.id, b]));

    type BotRow = {
      id: number; name: string; serverConfigId: number;
      serverConfig: { id: number; name: string; host: string };
      nickname: string; defaultChannel: string | null; voicePort: number | null;
      volume: number; autoStart: boolean; nowPlayingChannelId: string | null; createdAt: Date;
    };
    res.json((dbBots as BotRow[]).map((b) => {
      ...
      return {
        ...
        nowPlayingChannelId: b.nowPlayingChannelId ?? null,
        status: runtime?.status ?? 'stopped',
        nowPlaying: runtime?.nowPlaying ?? null,
        startError: runtime ? null : (manager.getStartError(b.id) ?? null),
...
musicBotRoutes.post('/', async (req: Request, res: Response, next) => {
  try {
    const manager: VoiceBotManager = req.app.locals.voiceBotManager;
    const { name, serverConfigId, nickname, serverPassword, defaultChannel, channelPassword, voicePort, volume, autoStart, nowPlayingChannelId } = req.body;
    ...
    const result = await manager.createBot({
      ...
      nowPlayingChannelId: nowPlayingChannelId ?? undefined,
    });
...
musicBotRoutes.put('/:id', async (req: Request, res: Response, next) => {
  try {
    ...
    const manager: VoiceBotManager = req.app.locals.voiceBotManager;
    const id = parseIntParam(req.params.id, 'id');
    const { name, nickname, serverPassword, defaultChannel, channelPassword, voicePort, volume, autoStart, nowPlayingChannelId } = req.body;
    ...
    const dbBot = await prisma.musicBot.update({
      ...
      data: {
        ...
        ...(nowPlayingChannelId !== undefined && { nowPlayingChannelId }),
      },
    });
...
musicBotRoutes.post('/:id/play-url', async (req: Request, res: Response, next) => {
  try {
    ...
    try {
      ...
      logger.error({ err: saveErr }, '[music-bots.routes] Failed to save music request history');
    }
...
musicBotRoutes.post('/:id/pause', async (req: Request, res: Response, next) => {
  try {
    const manager: VoiceBotManager = req.app.locals.voiceBotManager;
    const bot = manager.getBot(parseIntParam(req.params.id, 'id'));
    if (!bot) throw new AppError(404, 'Music bot not found');
    bot.pause();
    res.json({ success: true });
  } catch (err) { next(err); }
...
musicBotRoutes.post('/:id/queue', async (req: Request, res: Response, next) => {
  try {
    ...
    bot.queue.add({
      ...
      duration: song.duration ?? undefined,
      filePath: song.filePath,
      source: song.source as 'local' | 'youtube' | 'url' | 'radio',
      sourceUrl: song.sourceUrl ?? undefined,
    });
...
musicBotRoutes.post('/:id/queue/playlist', async (req: Request, res: Response, next) => {
  try {
    ...
    type PlaylistSongEntry = (typeof playlist.songs)[number];
    const items = playlist.songs.map((ps: PlaylistSongEntry) => ({
      id: String(ps.song.id),
      title: ps.song.title,
      artist: ps.song.artist ?? undefined,
      duration: ps.song.duration ?? undefined,
      filePath: ps.song.filePath,
      source: ps.song.source as 'local' | 'youtube' | 'url' | 'radio',
...
musicBotRoutes.delete('/:id/queue/:index', async (req: Request, res: Response, next) => {
  try {
    ...
    const index = parseIntParam(req.params.index, 'index');
...
musicBotRoutes.delete('/:id/stream/viewer/:clid', async (req: Request, res: Response, next) => {
  try {
    ...
    const bot = manager.getBot(parseIntParam(req.params.id, 'id'));
    if (!bot) throw new AppError(404, 'Music bot not found');
    await bot.kickVideoViewer(parseIntParam(req.params.clid, 'clid'));
    res.json({ success: true });
  } catch (err) { next(err); }
});
