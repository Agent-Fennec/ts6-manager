...
playlistRoutes.get('/', async (req: Request, res: Response, next) => {
  try {
    ...
    res.json((playlists as Array<{id: number; name: string; musicBotId: number|null; createdAt: Date; _count: {songs: number}}>).map((p) => ({
      id: p.id,
      name: p.name,
...
playlistRoutes.get('/:id', async (req: Request, res: Response, next) => {
  try {
    ...
    res.json({
      ...
      songs: (playlist.songs as Array<{position: number; song: {id: number; title: string; artist: string|null; duration: number|null; source: string; sourceUrl: string|null; fileSize: number|null; createdAt: Date}}>).map((ps) => ({
        id: ps.song.id,
        title: ps.song.title,
...
playlistRoutes.delete('/:id', async (req: Request, res: Response, next) => {
  try {
    const prisma = req.app.locals.prisma;
    await prisma.playlist.delete({ where: { id: parseIntParam(req.params.id, 'id') } });
    res.json({ success: true });
  } catch (err) { next(err); }
});
...
playlistRoutes.post('/:id/songs', async (req: Request, res: Response, next) => {
  try {
    const prisma = req.app.locals.prisma;
    const playlistId = parseIntParam(req.params.id, 'id');
...
playlistRoutes.delete('/:id/songs/:songId', async (req: Request, res: Response, next) => {
  try {
    const prisma = req.app.locals.prisma;
    const playlistId = parseIntParam(req.params.id, 'id');
    const songId = parseIntParam(req.params.songId, 'songId');
