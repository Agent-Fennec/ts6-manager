  ...
  try {
    ...
    const servers = await prisma.tsServerConfig.findMany({
      select: {
        ...
        useHttps: true, sshPort: true, sshUsername: true, enabled: true,
        createdAt: true, queryBotNickname: true, sshBotNickname: true, queryBotChannel: true,
      },
      orderBy: { id: 'asc' },
    });
...
serverRoutes.put('/:configId', requireRole('admin'), async (req: Request, res: Response, next) => {
  try {
    ...
    const fields = ['name', 'host', 'webqueryPort', 'apiKey', 'useHttps', 'sshPort', 'sshUsername', 'sshPassword', 'enabled', 'queryBotChannel', 'queryBotNickname', 'sshBotNickname'];
    ...
    // Apply nickname changes to live TS3 server immediately
    const botEngine = req.app.locals.botEngine;
    if (botEngine && (req.body.queryBotNickname !== undefined || req.body.sshBotNickname !== undefined || req.body.queryBotChannel !== undefined)) {
      botEngine.applyNicknameChanges(id, req.body.queryBotNickname, req.body.sshBotNickname, req.body.queryBotChannel).catch(() => {});
    }
