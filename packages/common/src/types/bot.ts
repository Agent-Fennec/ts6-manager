...
// --- Valkey (Redis) Actions ---
export interface ValkeyGetActionData {
  actionType: 'valkeyGet';
  label: string;
  key: string;
  storeAs?: string;
}

export interface ValkeySetActionData {
  actionType: 'valkeySet';
  label: string;
  key: string;
  value: string;
  ttlSeconds?: number;
...
export interface ValkeyDeleteActionData {
  actionType: 'valkeyDelete';
  label: string;
  key: string;
}

export interface GroupRemoveAllActionData {
  actionType: 'groupRemoveAll';
  label: string;
  keepGroupIds: string;
}

export interface GroupRestoreListActionData {
  actionType: 'groupRestoreList';
  label: string;
  groupIds: string;
  excludeGroupIds?: string;
}

export interface GenerateTokenActionData {
  actionType: 'generateToken';
  label: string;
  tokenType: '0' | '1';    // '0' = server group, '1' = channel group
  groupId: string;          // sgid (type 0) or cgid (type 1)
  channelId?: string;       // cid — required for type 1, ignored for type 0
  storeAs?: string;         // temp variable name to store the token string
}

export interface SetClientChannelGroupActionData {
